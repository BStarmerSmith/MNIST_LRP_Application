...
def e_lrp(model, img_tensor):
    ...
    layers.append(toconv(model._modules['fc1']))
    layers.append(toconv(model._modules['fc2']))
    ...
    for l in range(1,L)[::-1]:
        ...
        if isinstance(layers[l],torch.nn.Conv2d) or isinstance(layers[l],torch.nn.AvgPool2d):
            if l <= 5: rho = lambda p: p + 0.25*p.clamp(min=0); incr = lambda z: z+1e-9
            else: rho = lambda p: p; incr = lambda z: z+1e-9
...
def toconv(layer):
    ...
    if isinstance(layer, torch.nn.Linear):
        ...
        newlayer = torch.nn.Conv2d(m,n,14)
        newlayer.weight = torch.nn.Parameter(layer.weight.reshape(1,m,14,14))
